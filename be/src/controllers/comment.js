...
const addComment = async (req, res) => {
  ...
  try {
    ...
    if (existingComment) {
      return res
        .status(400)
        .json({ message: "Bạn đã bình luận cho đơn hàng này rồi" });
    }
    ...
    if (!user || !order) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy người dùng hoặc đơn hàng" });
    }
    ...
    if (!["delivered", "received"].includes(order.status)) {
      return res
        .status(400)
        .json({ message: "Bạn cần nhận đơn hàng trước khi bình luận" });
    }
    ...
    if (!product) {
      return res
        .status(400)
        .json({ message: "Sản phẩm này không có trong đơn hàng" });
    }
...
const checkReviewedProducts = async (req, res) => {
  ...
  if (!data || !Array.isArray(data)) {
    return res.status(400).json({
      message:
        "Payload không hợp lệ. Cần có mảng chứa orderId và productSlugs.",
    });
  }
...
const getCommentsByProducts = async (req, res) => {
  ...
  if (!productIds || productIds.length === 0) {
    return res
      .status(400)
      .json({ message: "Danh sách productIds không hợp lệ" });
  }
  ...
  try {
    ...
    const groupedComments = productIds.reduce((acc, productId) => {
      acc[productId] = comments.filter(
        (comment) => comment.productId.toString() === productId
      );
      return acc;
    }, {});
